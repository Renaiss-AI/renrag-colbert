...
class ColbertSearcher:
    ...
    def reload_index(self,
        ...
        try:
            ...
            self.model_name = model_name or self.metadata.get("model_name", "colbert-ir/colbertv2.0")
    ...
    def _load_document_embeddings(self):
        ...
        for doc_file in doc_files:
            try:
                ...
                if any(skip in doc_file for skip in ["doc_mapping.pt", "doc_metadata.pt", "doc_file_ids.pt"]):
                    continue
    ...
    def _score_documents(self, query_embedding: torch.Tensor) -> List[tuple]:
        ...
        for doc_id, doc_embedding in self.doc_embeddings.items():
            ...
            # Normalize score to get value between 0 and 1
            query_len = (query_embedding.norm(dim=1) > 0).sum().item()
            normalized_score = score / (query_len * 100) if query_len > 0 else 0
    ...
    def search(self, 
               ...
               k: int = 10,
               threshold: float = 0.0,
               filter_by_files: Optional[Union[str, List[str]]] = None) -> List[Dict[str, Any]]:
        """
        ...
        for doc_idx, score in filtered_scores:
            ...
            doc_id = self.document_ids.get(doc_idx, str(doc_idx))  # Fall back to index if no document ID
